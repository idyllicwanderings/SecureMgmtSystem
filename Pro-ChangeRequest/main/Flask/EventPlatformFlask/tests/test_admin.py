...
def test_case_12_ADMIN(app, request, user1, user2, freeuserrole, premiumuserrole, adminrole):
    with current_app.test_request_context(
        ...
        # Scenario: a user user2 with ADMIN role
        user1.role = freeuserrole
        db.session.add(user1)
        user2.role = adminrole
        db.session.add(user2)
        db.session.add(premiumuserrole)
        db.session.commit()
        # login
        login_user(user2)
        # Test ADMIN accessing update_user()
        result = update_user()
        assert user1.role.name == premiumuserrole.name            

#Test Case	13	Update Person.role from FREE to ADMIN
def test_case_13_ADMIN(app, request, user1, freeuserrole, user2, adminrole):
    with current_app.test_request_context(
        data={ "id": user1.id, 
              "name": user1.name,
              "surname": user1.surname,
        ...
        # Test ADMIN accessing update_user()
        result = update_user()
        assert user1.role.name == adminrole.name            

#Test Case	14	Read Person.username
def test_case_14_ADMIN(app, request, user1, adminrole):
    with current_app.test_request_context():
        # Scenario: a person user1 with ADMIN role.
        user1.role = adminrole
        db.session.add(user1)
        db.session.commit()
        # login
        login_user(user1)
        # Test ADMIN accessing user()
...
def test_case_16_ADMIN(app, request, user1, adminrole):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        user1.role = adminrole
        db.session.add(user1)
...
def test_case_19_ADMIN_1(app, request, user1, user2, adminrole, premiumuserrole, privateevent1, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 admin role, user2 with PREMIUMUSER role.
        #           a category category1
...
# Testing /events
def test_case_20_ADMIN(app, request, user1, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a public event owned by user1
        user1.role = adminrole
...
# Testing /profile
def test_case_20_ADMIN_1(app, request, user1, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: a admin user1, 
...
# Testing /view_event
def test_case_20_ADMIN_2(app, request, user1, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a public event owned by user1
        user1.role = adminrole
...
# Test edit_event
def test_case_20_ADMIN_3(app, request, user1, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a public event owned by user1
        user1.role = adminrole
        ...
        # Log in with user1
        login_user(user1)
        # Test ADMIN accessing edit_event()
        result = edit_event(publicevent1.id)
        assert result.get("event").title == publicevent1.title

# Test view_category
def test_case_20_ADMIN_4(app, request, user1, adminrole, publicevent1, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a category category1
        #           a public event owned by user1, of category1
        ...
        # Log in with user1
        login_user(user1)
        # Test ADMIN accessing view_category()
...
def test_case_21_ADMIN(app, request, user1, user2, adminrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
...
def test_case_21_ADMIN_1(app, request, user1, user2, premiumuserrole, adminrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a person user2 with PREMIUMUSER role.
...
# Testing /profile: 
def test_case_21_ADMIN_2(app, request, user1, premiumuserrole, adminrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
...
# Testing /view_event
def test_case_21_ADMIN_3(app, request, user1, user2, adminrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a person user2 with PREMIUMUSER role.
        #           a private event owned by user2
...
def test_case_21_ADMIN_4(app, request, user1, user2, adminrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a person user2 with PREMIUMUSER role.
...
# Test edit_event
def test_case_21_ADMIN_5(app, request, user1, user2, adminrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with ADMIN role.
        #           a person user2 with PREMIUMUSER role.
        #           a private event owned by user2
...
def test_case_21_ADMIN_6(app, request, user1, user2, adminrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with adminrole role.
        #           a person user2 with PREMIUMUSER role.
        #           a private event owned by user2, attended by user2
        user1.role = adminrole
        db.session.add(user1)
...
# Test view_category
def test_case_21_ADMIN_7(app, request, user1, user2, adminrole, premiumuserrole, privateevent1, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with ADMIN role.
        #           a person user2 with PREMIUMUSER role.
        #           a category category1
...
def test_case_21_ADMIN_8(app, request, user1, user2, adminrole, premiumuserrole, privateevent1, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with adminrole role.
        #           a person user2 with PREMIUMUSER role.
        #           a category category1
...
# Testing /view_event
def test_case_22_ADMIN(app, request, user1, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a public event owned by user1
        user1.role = adminrole
...
# Test edit_event
def test_case_22_ADMIN_1(app, request, user1, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a public event owned by user1
        user1.role = adminrole
...
# Testing /view_event
def test_case_23_ADMIN(app, request, user1, user2, adminrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a person user2 with PREMIUMUSER role.
        #           a private event owned by user2
...
def test_case_23_ADMIN_1(app, request, user1, user2, adminrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a person user2 with PREMIUMUSER role.
        #           a private event owned by user2, attended by user1
...
# Test edit_event
def test_case_23_ADMIN_2(app, request, user1, user2, adminrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a person user2 with PREMIUMUSER role.
        #           a private event owned by user2
...
def test_case_23_ADMIN_3(app, request, user1, user2, adminrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a person user2 with PREMIUMUSER role.
        #           a private event owned by user2, attended by user1
...
# Test \events
def test_case_24_ADMIN(app, request, user1, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a public event owned by user1
        user1.role = adminrole
...
# Test profile
def test_case_24_ADMIN_1(app, request, user1, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: a admin user1, 
        #           a publicevent of user1
        user1.role = adminrole
...
# Test \view_category
def test_case_24_ADMIN_2(app, request, user1, adminrole, publicevent1, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a category category1
        #           a public event owned by user1, of category1
...
# Test \events
def test_case_25_ADMIN(app, request, user1, user2, adminrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a person user2 with PREMIUMUSER role.
        #           a private event owned by user2
...
def test_case_25_ADMIN_1(app, request, user1, user2, adminrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a person user2 with PREMIUMUSER role.
        #           a private event owned by user2, attended by user1
...
# Test view_category
def test_case_25_ADMIN_2(app, request, user1, user2, adminrole, premiumuserrole, privateevent1, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a person user2 with PREMIUMUSER role.
        #           a category category1
...
def test_case_25_ADMIN_3(app, request, user1, user2, adminrole, premiumuserrole, privateevent1, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a person user2 with PREMIUMUSER role.
        #           a category category1
...
# Test \events
def test_case_26_ADMIN(app, request, user1, adminrole, category1):
    with current_app.test_request_context():
        # Scenario: a category1, a admin user1
        user1.role = adminrole
        db.session.add(user1)
...
# Test \profile:
# Not applicable because admin cannot subscribe to a category

# Test \categories
def test_case_26_ADMIN_1(app, request, user1, adminrole, category1):
    with current_app.test_request_context():
        # Scenario: a category category1, a admin user1
        user1.role = adminrole
        db.session.add(user1)
        db.session.add(category1)
        db.session.commit()
        # Log in with user1
        login_user(user1)
        # Test ADMIN accessing categories()
        result = categories()
        assert len(result.get("categories")) == 1
        assert result.get("categories")[0].name == category1.name

# Testing /view_event
def test_case_26_ADMIN_2(app, request, user1, adminrole, publicevent1, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a category category1
        #           a public event owned by user1, of category1
...
# Testing /view_event
def test_case_26_ADMIN_3(app, request, user1, user2, adminrole, premiumuserrole, privateevent1, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a person user2 with PREMIUMUSER role.
        #           a category category1
...
# Test edit_event
def test_case_26_ADMIN_4(app, request, user1, adminrole, publicevent1, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a category category1
        #           a public event owned by user1, of category1
...
# Test edit_event
def test_case_26_ADMIN_5(app, request, user1, user2, adminrole, premiumuserrole, privateevent1, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a person user2 with PREMIUMUSER role.
        #           a category category1
...
# Test /user
def test_case_26_ADMIN_6(app, request, user1, adminrole, moderatorrole, user2, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
...
# Test view_category
def test_case_26_ADMIN_7(app, request, user1, adminrole, category1):
    with current_app.test_request_context():
        # Scenario: a category category1 and a admin user1
        user1.role = adminrole
        db.session.add(user1)      
        db.session.add(category1)
        db.session.commit()
...
# Test edit_category
def test_case_26_ADMIN_8(app, request, user1, adminrole, category1):
    with current_app.test_request_context():
        # Scenario: a category category1 and a admin user1
        user1.role = adminrole
        db.session.add(user1)      
        ...
        # Log in with user1
        login_user(user1)
        # Test ADMIN accessing edit_category()
        result = edit_category(category1.id)
        assert result.get("category").name == category1.name

#Test Case	27	Read Category.moderators
# Test edit_category
def test_case_27_ADMIN(app, request, user1, adminrole, moderatorrole, user2, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a person user2 with MODERATOR role.
        #           a category category1
...
# Test \categories
def test_case_28_ADMIN(app, request, user1, adminrole, publicevent1, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a category category1
        #           a public event owned by user1, of category1
...
# Test view_category
def test_case_28_ADMIN_1(app, request, user1, adminrole, publicevent1, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a category category1
        #           a public event owned by user1, of category1
...
def test_case_29_ADMIN(app, request, user1, adminrole, publicevent1):
    with current_app.test_request_context(
...
# Testing /events
def test_case_30_ADMIN(app, request, user1, user2, freeuserrole, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role, user2 with FREEUSER role.
...
# Test view_event
def test_case_30_ADMIN_1(app, request, user1, user2, freeuserrole, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role, user2 with FREEUSER role.
        #           a public event owned by user2
        user1.role = adminrole
...
# Test edit_event
def test_case_30_ADMIN_2(app, request, user1, user2, freeuserrole, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role, user2 with FREEUSER role.
        #           a public event owned by user2
        user1.role = adminrole
...
# Test view_category
def test_case_30_ADMIN_3(app, request, user1, user2, freeuserrole, adminrole, publicevent1, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role, user2 with FREEUSER role.
        #           a category category1
        #           a public event owned by user2, of category1
...
# Testing /events
def test_case_31_ADMIN(app, request, user1, user2, adminrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a person user2 with PREMIUMUSER role.
        #           a private event owned by user2
...
# Test view_event
def test_case_31_ADMIN_1(app, request, user1, user2, adminrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a person user2 with PREMIUMUSER role.
        #           a private event owned by user2
...
# Test edit_event
def test_case_31_ADMIN_2(app, request, user1, user2, adminrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a person user2 with PREMIUMUSER role.
        #           a private event owned by user2
...
# Test view_category
def test_case_31_ADMIN_3(app, request, user1, 
                           user2, adminrole, privateevent1, premiumuserrole, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a person user2 with PREMIUMUSER role.
        #           a category category1
...
# Test view_event
def test_case_32_ADMIN(app, request, user1, user2, freeuserrole, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with admin role, user2 with FREEUSER role.
        #           a public event owned by user2
        user1.role = adminrole
        db.session.add(user1)
        user2.role = freeuserrole
...
# Test manage_event
def test_case_32_ADMIN_1(app, request, user1, user2, freeuserrole, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with admin role, user2 with FREEUSER role.
        #           a public event owned by user2
        user1.role = adminrole
        ...
        # Log in with user1
        login_user(user1)
        # Test ADMIN accessing manage_event()
        try:
...
# Test view_event
def test_case_33_ADMIN(app, request, user1, adminrole, premiumuserrole, 
                         user2, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a person user2 with PREMIUMUSER role.
...
# Test manage_event
def test_case_33_ADMIN_1(app, request, user1, adminrole, premiumuserrole, 
                         user2, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a person user2 with PREMIUMUSER role.
...
# Test view_event
def test_case_34_ADMIN(app, request, user1, adminrole, premiumuserrole,
                         user2, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with adminrole role.
        #           a person user2 with PREMIUMUSER role.
...
# Test manage_event
def test_case_34_ADMIN_1(app, request, user1, premiumuserrole, adminrole,
                         user2, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with adminrole role.
        #           a person user2 with PREMIUMUSER role.
...
def test_case_35_ADMIN(app, request, user1, user2, freeuserrole, adminrole, publicevent1, category1):
    with current_app.test_request_context(
        ...
        # Log in with user1
        login_user(user1)
        # Test ADMIN accessing update_event()
        result = update_event()
...
def test_case_36_ADMIN(app, request, user1, user2, freeuserrole, adminrole, publicevent1, category1):
    with current_app.test_request_context(
        data={ "id": publicevent1.id, 
              "title": "new title", 
              "description": "new description", 
...
def test_case_36_ADMIN_1(app, request, user1, user2, freeuserrole, adminrole, publicevent1, category1):
    with current_app.test_request_context(
        data={ "id": publicevent1.id, 
              "title": "new title", 
              "description": "new description", 
              "categories": publicevent1.categories}):
        # Scenario: person user1 ADMIN role, user2 with FREEUSER role.
        #           a public event owned by user2
        #           attended by user1
        user1.role = adminrole
        db.session.add(user1)
...
def test_case_38_ADMIN(app, request, user1, user2, freeuserrole, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with admin role, user2 with FREEUSER role.
        ...
        # Log in with user1
        login_user(user1)
        # Test ADMIN accessing promote_manager()
...
def test_case_39_ADMIN(app, request, user1, user2, freeuserrole, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with admin role, user2 with FREEUSER role.
        ...
        # Log in with user1
        login_user(user1)
        # Test ADMIN accessing demote_manager()
...
def test_case_40_ADMIN(app, request, user1, user2, user3, freeuserrole, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with admin role, user2, user3 with FREEUSER role.
...
def test_case_40_ADMIN_1(app, request, user1, user2, user3, freeuserrole, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 ADMIN role, user2, user3 with FREEUSER role.
        #           a public event owned by user3, managed by user2
...
def test_case_40_ADMIN_2(app, request, user1, user2, user3, freeuserrole, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 ADMIN role, user2, user3 with FREEUSER role.
        #           a public event owned by user3, managed by user2
...
def test_case_41_ADMIN(app, request, user1, user2, freeuserrole, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with admin role, user2 with FREEUSER role.
        ...
        # Log in with user1
        login_user(user1)
        # Test ADMIN accessing accept_request()
...
def test_case_42_ADMIN(app, request, user1, user2, user3, freeuserrole, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with admin role, user2, user3 with FREEUSER role.
...
def test_case_43_ADMIN(app, request, user1, user2, user3, freeuserrole, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with admin role, user2, user3 with FREEUSER role.
        #           a public event owned by user3, 
        #           request to join by user2
...
def test_case_43_ADMIN_1(app, request, user1, user2, user3, freeuserrole, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with admin role, user2, user3 with FREEUSER role.
        #           a public event owned by user3, 
        #           request to join by user2
...
def test_case_44_ADMIN(app, request, user1, user2, freeuserrole, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with admin role, user2 with FREEUSER role.
        #           a public event owned by user2,
        #           attended by user1
...
def test_case_44_ADMIN_1(app, request, user1, user2, adminrole, freeuserrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 ADMIN role, user2 with FREEUSER role.
        #           a public event owned by user2,
        #           NOT attended by user1
...
def test_case_45_ADMIN(app, request, user1, user2, freeuserrole, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with admin role, user2 with FREEUSER role.
        #           a public event owned by user1, request to join by user2
        user1.role = adminrole
        ...
        # Log in with user1
        login_user(user1)
        # Test ADMIN accessing reject_request()
        result = reject_request(user2.id, publicevent1.id)
        assert user2.id not in [m.id for m in publicevent1.attendants]
        assert user2.id not in [m.id for m in publicevent1.requesters]

#Test Case	46	Reject request to join manager
def test_case_46_ADMIN(app, request, user1, user2, user3, freeuserrole, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with admin role, user2, user3 with FREEUSER role.
        #           a public event owned by user3, 
        #           request to join by user2
...
#def test_case_47_ADMIN(app, request, user1, user2, freeuserrole, adminrole, publicevent1):
#    with current_app.test_request_context():
#        # Scenario: person user1 with admin role, user2 with FREEUSER role.
#        #           a public event owned by user2,
#        #           attended by user1
...
#        # Log in with user1
#        login_user(user1)
#        # Test ADMIN accessing leave()
#        result = leave(publicevent1.id)
#        assert user1.id not in [m.id for m in publicevent1.attendants]

#def test_case_47_ADMIN_1(app, request, user1, user2, freeuserrole, adminrole, publicevent1):
#    with current_app.test_request_context():
#        # Scenario: person user1 with admin role, user2 with FREEUSER role.
#        #           a public event owned by user2,
#        #           attended by user1
...
#        # Log in with user1
#        login_user(user1)
#        # Test ADMIN accessing remove_attendee()
#        result = remove_attendee(user1.id, publicevent1.id)
#        assert user1.id not in [m.id for m in publicevent1.attendants]

##Test Case	48	Remove others from attending an event, manager
def test_case_48_ADMIN(app, request, user1, user2, user3, freeuserrole, 
                             adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with admin role, user2, user3 with FREEUSER role.
        #           a public event owned by user2,
        #           managed by user1
...
def test_case_49_ADMIN(app, request, user1, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with admin role.
        #           a public event owned by user1
        user1.role = adminrole
        db.session.add(user1)
        publicevent1.owner = user1 
...
def test_case_50_ADMIN(app, request, user1, user2, user3, freeuserrole, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with admin role, user2, user3 with FREEUSER role.
        #           a public event owned by user2,
...
def test_case_51_ADMIN(app, request, user1, user2, freeuserrole, 
                             adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with admin role, user2 with FREEUSER role.
        #           a public event owned by user2
...
def test_case_52_ADMIN(app, request, user1, user2, adminrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with admin role, 
...
def test_case_53_ADMIN(app, request, user1, user2, user3, freeuserrole, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with admin role, user2, user3 with FREEUSER role.
        #           a public event owned by user3, 
        #           NOT request to join by user2
        user1.role = adminrole
        db.session.add(user1)
        user2.role = freeuserrole
        db.session.add(user2)
...
#def test_case_54_ADMIN(app, request, user1, user2, freeuserrole, adminrole, publicevent1):
#    with current_app.test_request_context():
#        # Scenario: person user1 with admin role, user2 with FREEUSER role.
#        #           a public event owned by user2, 
...
# Test /manage_event
def test_case_56_ADMIN(app, request, user1, user2, user3, freeuserrole, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
...
# Test \users
def test_case_56_ADMIN_1(app, request, user1, user2, freeuserrole, adminrole):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role, user2 with FREEUSER role.
        user1.role = adminrole
        db.session.add(user1)
...
# Test \user
def test_case_56_ADMIN_2(app, request, user1, user2, freeuserrole, adminrole):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role, user2 with FREEUSER role.
        user1.role = adminrole
        db.session.add(user1)
...
# Test edit_category
def test_case_56_ADMIN_3(app, request, user1, adminrole, moderatorrole,
...
# Test /main
def test_case_57_ADMIN(app, request, user1, adminrole):
    with current_app.test_request_context():
        # Scenario: a admin user1
        user1.role = adminrole
        db.session.add(user1)
...
# Test /manage_event
def test_case_57_ADMIN_1(app, request, user1, freeuserrole, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a public event owned by user1
        user1.role = adminrole
...
# Test \users
def test_case_57_ADMIN_1(app, request, user1, adminrole):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        user1.role = adminrole
        db.session.add(user1)
...
# Test \user
def test_case_57_ADMIN_2(app, request, user1, adminrole):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        user1.role = adminrole
        db.session.add(user1)
...
def test_case_58_ADMIN(app, request, user1, adminrole):
    with current_app.test_request_context(
        data={"id": user1.id, 
              "name": "newname",
              "surname": "newsurname",
              "role": adminrole.name}):
        # Scenario: a user user1 with admin role
        user1.role = adminrole
        db.session.add(user1)
        db.session.commit()
        # Log in with user1
...
def test_case_59_ADMIN(app, request, user1, user2, freeuserrole, adminrole, premiumuserrole):
    with current_app.test_request_context(
        data={"id": user2.id, 
              "name": "newname",
              "surname": "newsurname",
...
# Test /profile
def test_case_61_ADMIN(app, request, user1, freeuserrole, adminrole, premiumuserrole, 
                          user2, publicevent1, category1):
    with current_app.test_request_context():
        # Scenario: a user1 with admin role, user2 with premiumrole
        #           a category category1
...
# Test /profile
def test_case_62_ADMIN(app, request, user1, freeuserrole, adminrole,
                          user2, publicevent1, category1):
    with current_app.test_request_context():
        # Scenario: a user1 with admin role, user2 with premiumrole
        #           a category category1
...
def test_case_63_ADMIN(app, request, user1, adminrole, freeuserrole):
    with current_app.test_request_context(
        data={ "id": user1.id, 
              "name": user1.name,
              "surname": user1.surname,
...
def test_case_64_ADMIN(app, request, user1, user2, adminrole, freeuserrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 admin role, user2 with FREEUSER role.
        #           a public event owned by user1, request to join by user2
        user1.role = adminrole
...
def test_case_65_ADMIN(app, request, user1, adminrole, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with adminrole role.
...
def test_case_66_ADMIN(app, request, user1, adminrole, category1):
    with current_app.test_request_context():
        # Scenario: a category category1
        #           a person user1 with admin role.
        ...
        # Log in with user1
        login_user(user1)
        # Test ADMIN accessing unsubscribe()
...
def test_case_67_68_ADMIN(app, request, user1, adminrole, category1):
    with current_app.test_request_context():
        # Scenario: a category category1
        #           a person user1 with admin role.
        user1.role = adminrole
...
def test_case_70_ADMIN(app, request, user1, user2, adminrole, publicevent1, category1):
    with current_app.test_request_context():
        # Scenario: person user1, user2 with admin role.
        #           a category category1
        #           a public event owned by user2, of category1
        user1.role = adminrole
        db.session.add(user1)
        ...
        # Log in with user1
        login_user(user1)
        # Test ADMIN accessing remove_category()
        try:
...
def test_case_70_ADMIN_1(app, request, user1, user2, adminrole, publicevent1, category1):
    with current_app.test_request_context(
        data={ "id": publicevent1.id, 
...
# Not applicable, ADMIN cannot be moderator at the first place

#Test Case	73	Remove category moderated from event
# Not applicable, ADMIN cannot be moderator at the first place

#Test Case	74	Read category subscribers
def test_case_74_ADMIN(app, request, user1, premiumuserrole, 
                               adminrole, category1, 
                               user2):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
...
def test_case_74_ADMIN_1(app, request, user1, 
                               adminrole, category1, 
...
# Duplicated test case 10-13

#Test Case	78	Add user as moderator category
def test_case_78_ADMIN(app, request, user1, user2, adminrole, moderatorrole, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a person user2 with MODERATOR role.
        #           a category category1
        ...
        # Log in with user1
        login_user(user1)
        # Test ADMIN accessing add_moderator()
        result = add_moderator(user2.id, category1.id)
        assert len(category1.moderators) == 1 and \
               category1.moderators[0].id == user2.id

#Test Case	79	Remove user as moderator category
def test_case_79_ADMIN(app, request, user1, user2, adminrole, moderatorrole, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a person user2 with MODERATOR role.
        ...
        # Log in with user1
        login_user(user1)
        # Test ADMIN accessing remove_moderator()
        result = remove_moderator(user2.id, category1.id)
        assert len(category1.moderators) == 0

#Test Case	80	Create Category
def test_case_80_ADMIN(app, request, user1, adminrole, category1):
    with current_app.test_request_context(
        ...
        # Log in with user1
        login_user(user1)
        # Test ADMIN accessing create_category()
...
def test_case_82_ADMIN(app, request, user1, adminrole, category1):
    with current_app.test_request_context(
        ...
        # Log in with user1
        login_user(user1)
        # Test ADMIN accessing update_category()
        result = update_category()
        assert category1.name == "newname"

#Test Case	83	As a manager add attendant that has not requested
def test_case_83_ADMIN(app, request, user1, user2, user3, freeuserrole, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with admin role, user2, user3 with FREEUSER role.
        #           a public event owned by user3
        #           managed by user1
...
def test_case_84_ADMIN(app, request, user1, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with admin role.
        #           a public event owned by user1
        user1.role = adminrole
...
# leave functionality is incorrectly implemented, waive this test
#def test_case_86_ADMIN(app, request, user1, user2, freeuserrole, adminrole, publicevent1):
#    with current_app.test_request_context():
#        # Scenario: person user1 with admin role, user2 with FREEUSER role.
#        #           a public event owned by user2, managed by user1
#        user1.role = adminrole
...
def test_case_86_ADMIN_1(app, request, user1, user2, freeuserrole, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with admin role, user2 with FREEUSER role.
        #           a public event owned by user2, managed by user1
        user1.role = adminrole
...
def test_case_87_ADMIN(app, request, user1, user2, freeuserrole, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with admin role, user2 with FREEUSER role.
        #           a public event owned by user2, managed by user1
        user1.role = adminrole
...
#def test_case_88_ADMIN(app, request, user1, user2, adminrole, moderatorrole, premiumuserrole, category1):
#    with current_app.test_request_context(
#        data={ "id": user2.id, 
#              "name": user2.name,
#              "surname": user2.surname,
#              "role": premiumuserrole.name}):
#        # Scenario: a person user2 with moderator role.
#        #           a category category1, moderated by user2
#        user1.role = adminrole
#        db.session.add(user1)
#        user2.role = moderatorrole
...
#            assert True
#        finally:
#            assert user2.role.name == moderatorrole.name

#Test Case	90	Accept someone else's request, not owner not manager
def test_case_90_ADMIN(app, request, user1, user2, user3, freeuserrole, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 admin role, user2, user3 with FREEUSER role.
        ...
        # Log in with user1
        login_user(user1)
        # Test adminrole accessing accept_request()
        try:
            result = accept_request(user2.id, publicevent1.id)
        except SecurityException:
            assert True
...
def test_case_91_ADMIN(app, request, user1, adminrole, user2, freeuserrole, publicevent1, category1):
    with current_app.test_request_context(
        data={ "id": publicevent1.id, 
              "title": publicevent1.title, 
              "description": publicevent1.description, 
              "categories": []}):
        # Scenario: a person user1 admin role, user2 with FREEUSER role.
        #           a public event owned by user2, managed by user1
        user1.role = adminrole
        db.session.add(user1)
        user2.role = freeuserrole
...
def test_case_92_ADMIN(app, request, user1, adminrole, user2, freeuserrole, publicevent1, category1):
    with current_app.test_request_context(
        data={ "id": publicevent1.id, 
              "title": publicevent1.title, 
              "description": publicevent1.description, 
...
def test_case_93_ADMIN(app, request, user1, user2, adminrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a person user2 with PREMIUMUSER role.
        #           a private event owned by user2
...
def test_case_93_ADMIN_1(app, request, user1, user2, adminrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a person user2 with PREMIUMUSER role.
        #           a private event owned by user2
...
def test_case_94_ADMIN(app, request, user1, adminrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with admin role.
        #           a public event owned by user1
        user1.role = adminrole

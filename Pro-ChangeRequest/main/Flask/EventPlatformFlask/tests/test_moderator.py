...
def test_case_12_MODERATOR(app, request, user1, user2, freeuserrole, premiumuserrole, moderatorrole):
    with current_app.test_request_context(
        ...
        # Scenario: a user user2 with MODERATOR role
        ...
        # login
        login_user(user2)
        # Test MODERATOR accessing update_user()
        try:
            result = update_user()
...
def test_case_13_MODERATOR(app, request, user1, freeuserrole, user2, moderatorrole, adminrole):
    with current_app.test_request_context(
        data={ "id": user1.id, 
              "name": user1.name,
              "surname": user1.surname,
...
def test_case_14_MODERATOR(app, request, user1, moderatorrole):
    with current_app.test_request_context():
...
def test_case_16_MODERATOR(app, request, user1, moderatorrole):
    with current_app.test_request_context():
        ...
        # Test MODERATOR accessing user()
        result = user(user1.id)
        assert result.get("user").role.name == moderatorrole.name

#Test Case	17	Read Person.moderates
# Not applicable No endpoints display this information

#Test Case	18	Read Event.private
def test_case_18_MODERATOR(app, request, user1, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with MODERATOR role.
        ...
        # login
        login_user(user1)
        # Test MODERATOR accessing view_event(): 
        try:
...
def test_case_19_MODERATOR(app, request, user1, moderatorrole, publicevent1, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with MODERATOR role.
...
def test_case_19_MODERATOR_1(app, request, user1, user2, moderatorrole, premiumuserrole, privateevent1, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 MODERATOR role, user2 with PREMIUMUSER role.
        #           a category category1
...
# Testing /events
def test_case_20_MODERATOR(app, request, user1, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a public event owned by user1
        user1.role = moderatorrole
        ...
        # login
        login_user(user1)
        # Test MODERATOR accessing events():
        result = events()
...
# Testing /profile
def test_case_20_MODERATOR_1(app, request, user1, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: a moderator user1, 
        ...
        # Log in with user1
        login_user(user1)
        # Test MODERATOR accessing profile()
        result = profile()
...
# Testing /view_event
def test_case_20_MODERATOR_2(app, request, user1, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a public event owned by user1
        user1.role = moderatorrole
...
# Test edit_event
def test_case_20_MODERATOR_3(app, request, user1, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a public event owned by user1
        user1.role = moderatorrole
        ...
        # Log in with user1
        login_user(user1)
        # Test MODERATOR accessing edit_event()
        result = edit_event(publicevent1.id)
        assert result.get("event").title == publicevent1.title

# Test view_category
def test_case_20_MODERATOR_4(app, request, user1, moderatorrole, publicevent1, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a category category1
        #           a public event owned by user1, of category1
        ...
        # Log in with user1
        login_user(user1)
        # Test MODERATOR accessing view_category()
...
def test_case_21_MODERATOR(app, request, user1, user2, moderatorrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
...
def test_case_21_MODERATOR_1(app, request, user1, user2, premiumuserrole, moderatorrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a person user2 with PREMIUMUSER role.
        #           a private event owned by user2, attended by user1
        user1.role = moderatorrole
        db.session.add(user1)
        user2.role = premiumuserrole
        db.session.add(user2)
        ...
        # Log in with user1
        login_user(user1)
        # Test PREMIUMUSER accessing events():
        result = events()
        events_dto = result.get("events")
        assert len(events_dto) == 1 and \
               events_dto[0].title == privateevent1.title

# Testing /profile: 
def test_case_21_MODERATOR_2(app, request, user1, premiumuserrole, moderatorrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with MODERATOR role.
        ...
        # Log in with user1
        login_user(user1)
        # Test PREMIUMUSER accessing profile():
        result = profile()
        user_dto = result.get("user")
        assert len(user_dto.manages) == 1
        assert user_dto.manages[0].title == privateevent1.title
        assert len(user_dto.attends) == 1
        assert user_dto.attends[0].title == privateevent1.title

# Testing /view_event
def test_case_21_MODERATOR_3(app, request, user1, user2, moderatorrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a person user2 with PREMIUMUSER role.
        #           a private event owned by user2
...
def test_case_21_MODERATOR_4(app, request, user1, user2, moderatorrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a person user2 with PREMIUMUSER role.
        #           a private event owned by user2, attended by user2
        user1.role = moderatorrole
        db.session.add(user1)
        user2.role = premiumuserrole
        db.session.add(user2)
        ...
        # Log in with user1
        login_user(user1)
        # Test PREMIUMUSER accessing view_event(): 
        try:
            result = view_event(privateevent1.id)
            assert is_response(result) or \
                   result.get("event").title == privateevent1.title
        except SecurityException:
            assert True

# Test edit_event
def test_case_21_MODERATOR_5(app, request, user1, user2, moderatorrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with PREMIUMUSER role.
        #           a person user2 with PREMIUMUSER role.
        #           a private event owned by user2
...
def test_case_21_MODERATOR_6(app, request, user1, user2, moderatorrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderatorrole role.
        #           a person user2 with PREMIUMUSER role.
        #           a private event owned by user2, attended by user2
        user1.role = moderatorrole
        db.session.add(user1)
        ...
        # Log in with user1
        login_user(user1)
        # Test PREMIUMUSER accessing edit_event()
        try:
            result = edit_event(privateevent1.id)
            assert is_response(result) or \
                   result.get("event").title == privateevent1.title
...
# Test view_category
def test_case_21_MODERATOR_7(app, request, user1, user2, moderatorrole, premiumuserrole, privateevent1, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with PREMIUMUSER role.
        #           a person user2 with PREMIUMUSER role.
        #           a category category1
...
def test_case_21_MODERATOR_8(app, request, user1, user2, moderatorrole, premiumuserrole, privateevent1, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderatorrole role.
        #           a person user2 with PREMIUMUSER role.
        #           a category category1
        #           a public event owned by user2, of category1
        #           user1 attends this event
        user1.role = moderatorrole
        db.session.add(user1)
        user2.role = premiumuserrole
        db.session.add(user2)
        ...
        # Log in with user1
        login_user(user1)
        # Test PREMIUMUSER accessing view_category()
        try:
            result = view_category(category1.id)
            assert is_response(result) or \
                   (len(result.get("category").events) == 1 and \
                    result.get("category").events[0].title == privateevent1.title)
        except SecurityException:
            assert True

#Test Case	22	Read Event.description of a public event
# Testing /view_event
def test_case_22_MODERATOR(app, request, user1, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a public event owned by user1
        user1.role = moderatorrole
...
# Test edit_event
def test_case_22_MODERATOR_1(app, request, user1, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a public event owned by user1
        user1.role = moderatorrole
...
# Testing /view_event
def test_case_23_MODERATOR(app, request, user1, user2, moderatorrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a person user2 with PREMIUMUSER role.
        #           a private event owned by user2
...
def test_case_23_MODERATOR_1(app, request, user1, user2, moderatorrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a person user2 with PREMIUMUSER role.
        #           a private event owned by user2, attended by user1
        ...
        try:
            result = view_event(privateevent1.id)
            assert is_response(result) or \
                   result.get("event").description == privateevent1.description
        except SecurityException:
            assert True

# Test edit_event
def test_case_23_MODERATOR_2(app, request, user1, user2, moderatorrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a person user2 with PREMIUMUSER role.
        #           a private event owned by user2
...
def test_case_23_MODERATOR_3(app, request, user1, user2, moderatorrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a person user2 with PREMIUMUSER role.
        #           a private event owned by user2, attended by user1
...
# Test \events
def test_case_24_MODERATOR(app, request, user1, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a public event owned by user1
        user1.role = moderatorrole
...
# Test profile
def test_case_24_MODERATOR_1(app, request, user1, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: a moderator user1, 
        #           a publicevent of user1
        user1.role = moderatorrole
...
# Test \view_category
def test_case_24_MODERATOR_2(app, request, user1, moderatorrole, publicevent1, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a category category1
        #           a public event owned by user1, of category1
...
# Test \events
def test_case_25_MODERATOR(app, request, user1, user2, moderatorrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a person user2 with PREMIUMUSER role.
        #           a private event owned by user2
...
def test_case_25_MODERATOR_1(app, request, user1, user2, moderatorrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a person user2 with PREMIUMUSER role.
        #           a private event owned by user2, attended by user1
        ...
        events_dto = result.get("events")
        assert len(events_dto) == 1 and \
               events_dto[0].owner.fullname == user2.fullname

# Test view_category
def test_case_25_MODERATOR_2(app, request, user1, user2, moderatorrole, premiumuserrole, privateevent1, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a person user2 with PREMIUMUSER role.
        #           a category category1
...
def test_case_25_MODERATOR_3(app, request, user1, user2, moderatorrole, premiumuserrole, privateevent1, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a person user2 with PREMIUMUSER role.
        #           a category category1
        #           a public event owned by user2, of category1
        #           user1 attedns this event
        user1.role = moderatorrole
        db.session.add(user1)
        user2.role = premiumuserrole
        db.session.add(user2)
...
# Test \events
def test_case_26_MODERATOR(app, request, user1, moderatorrole, category1):
    with current_app.test_request_context():
        # Scenario: a category1, a moderator user1
        user1.role = moderatorrole
        db.session.add(user1)
...
# Test \profile:
# Not applicable because moderator cannot subscribe to a category

# Test \categories
def test_case_26_MODERATOR_1(app, request, user1, moderatorrole, category1):
    with current_app.test_request_context():
        # Scenario: a category category1, a moderator user1
        user1.role = moderatorrole
        db.session.add(user1)
        db.session.add(category1)
        db.session.commit()
        # Log in with user1
        login_user(user1)
        # Test MODERATOR accessing categories()
        result = categories()
        assert len(result.get("categories")) == 1
        assert result.get("categories")[0].name == category1.name

# Testing /view_event
def test_case_26_MODERATOR_2(app, request, user1, moderatorrole, publicevent1, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a category category1
        #           a public event owned by user1, of category1
...
# Testing /view_event
def test_case_26_MODERATOR_3(app, request, user1, user2, moderatorrole, premiumuserrole, privateevent1, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a person user2 with PREMIUMUSER role.
        #           a category category1
...
# Test edit_event
def test_case_26_MODERATOR_4(app, request, user1, moderatorrole, publicevent1, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a category category1
        #           a public event owned by user1, of category1
...
# Test edit_event
def test_case_26_MODERATOR_5(app, request, user1, user2, moderatorrole, premiumuserrole, privateevent1, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a person user2 with PREMIUMUSER role.
        #           a category category1
...
# Test /user
def test_case_26_MODERATOR_8(app, request, user1, moderatorrole, user2, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
...
# Test view_category
def test_case_26_MODERATOR_6(app, request, user1, moderatorrole, category1):
    with current_app.test_request_context():
        # Scenario: a category category1 and a moderator user1
        user1.role = moderatorrole
        db.session.add(user1)      
        db.session.add(category1)
        db.session.commit()
...
# Test edit_category
def test_case_26_MODERATOR_7(app, request, user1, moderatorrole, category1):
    with current_app.test_request_context():
        # Scenario: a category category1 and a moderator user1
        user1.role = moderatorrole
        db.session.add(user1)      
        ...
        # Log in with user1
        login_user(user1)
        # Test MODERATOR accessing edit_category()
        try:
...
# Test edit_category
def test_case_27_MODERATOR(app, request, user1, moderatorrole, user2, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a person user2 with MODERATOR role.
        #           a category category1
...
# Test \categories
def test_case_28_MODERATOR(app, request, user1, moderatorrole, publicevent1, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a category category1
        #           a public event owned by user1, of category1
...
# Test view_category
def test_case_28_MODERATOR_1(app, request, user1, moderatorrole, publicevent1, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a category category1
        #           a public event owned by user1, of category1
...
def test_case_29_MODERATOR(app, request, user1, moderatorrole, publicevent1):
    with current_app.test_request_context(
...
# Testing /events
def test_case_30_MODERATOR(app, request, user1, user2, freeuserrole, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with MODERATOR role, user2 with FREEUSER role.
...
# Test view_event
def test_case_30_MODERATOR_1(app, request, user1, user2, freeuserrole, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role, user2 with FREEUSER role.
        #           a public event owned by user2
        user1.role = moderatorrole
...
# Test edit_event
def test_case_30_MODERATOR_2(app, request, user1, user2, freeuserrole, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role, user2 with FREEUSER role.
        #           a public event owned by user2
        user1.role = moderatorrole
...
# Test view_category
def test_case_30_MODERATOR_3(app, request, user1, user2, freeuserrole, moderatorrole, publicevent1, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role, user2 with FREEUSER role.
        #           a category category1
        #           a public event owned by user2, of category1
...
# Testing /events
def test_case_31_MODERATOR(app, request, user1, user2, moderatorrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a person user2 with PREMIUMUSER role.
        #           a private event owned by user2
...
# Test view_event
def test_case_31_MODERATOR_1(app, request, user1, user2, moderatorrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a person user2 with PREMIUMUSER role.
        #           a private event owned by user2
...
# Test edit_event
def test_case_31_MODERATOR_2(app, request, user1, user2, moderatorrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a person user2 with PREMIUMUSER role.
        #           a private event owned by user2
...
# Test view_category
def test_case_31_MODERATOR_3(app, request, user1, 
                           user2, moderatorrole, privateevent1, premiumuserrole, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a person user2 with PREMIUMUSER role.
        #           a category category1
...
# Test view_event
def test_case_32_MODERATOR(app, request, user1, user2, freeuserrole, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with moderator role, user2 with FREEUSER role.
        #           a public event owned by user2
        user1.role = moderatorrole
        db.session.add(user1)
        user2.role = freeuserrole
...
# Test manage_event
def test_case_32_MODERATOR_1(app, request, user1, user2, freeuserrole, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with moderator role, user2 with FREEUSER role.
        #           a public event owned by user2
        user1.role = moderatorrole
        ...
        # Log in with user1
        login_user(user1)
        # Test MODERATOR accessing manage_event()
        try:
...
# Test view_event
def test_case_33_MODERATOR(app, request, user1, moderatorrole, premiumuserrole, 
                         user2, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a person user2 with PREMIUMUSER role.
...
# Test manage_event
def test_case_33_MODERATOR_1(app, request, user1, moderatorrole, premiumuserrole, 
                         user2, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a person user2 with PREMIUMUSER role.
...
# Test view_event
def test_case_34_MODERATOR(app, request, user1, moderatorrole, premiumuserrole,
                         user2, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderatorrole role.
        #           a person user2 with PREMIUMUSER role.
        ...
        try:
            result = view_event(privateevent1.id)
            assert is_response(result) or \
                   len(result.get("event").attendants) == 2 and \
                   ((result.get("event").attendants[0].fullname == user1.fullname and \
                     result.get("event").attendants[1].fullname == user2.fullname) or \
                    (result.get("event").attendants[0].fullname == user2.fullname and \
                     result.get("event").attendants[1].fullname == user1.fullname))
        except SecurityException:
            assert True

# Test manage_event
def test_case_34_MODERATOR_1(app, request, user1, premiumuserrole, moderatorrole,
                         user2, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderatorrole role.
        #           a person user2 with PREMIUMUSER role.
        ...
        # Log in with user1
        login_user(user1)
        # Test PREMIUMUSER accessing manage_event()
        try:
            result = manage_event(privateevent1.id)
            assert is_response(result) or \
                   len(result.get("event").attendants) == 2 and \
...
def test_case_35_MODERATOR(app, request, user1, user2, freeuserrole, moderatorrole, publicevent1, category1):
    with current_app.test_request_context(
        ...
        # Log in with user1
        login_user(user1)
        # Test MODERATOR accessing update_event()
        result = update_event()
...
def test_case_36_MODERATOR(app, request, user1, user2, freeuserrole, moderatorrole, publicevent1, category1):
    with current_app.test_request_context(
        data={ "id": publicevent1.id, 
              "title": "new title", 
              "description": "new description", 
...
def test_case_36_MODERATOR_1(app, request, user1, user2, freeuserrole, moderatorrole, publicevent1, category1):
    with current_app.test_request_context(
        data={ "id": publicevent1.id, 
              "title": "new title", 
              "description": "new description", 
              "categories": publicevent1.categories}):
        # Scenario: person user1 MODERATOR role, user2 with FREEUSER role.
        #           a public event owned by user2
        #           attended by user1
        user1.role = moderatorrole
        db.session.add(user1)
...
def test_case_38_MODERATOR(app, request, user1, user2, freeuserrole, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with moderator role, user2 with FREEUSER role.
        ...
        # Log in with user1
        login_user(user1)
        # Test MODERATOR accessing promote_manager()
...
def test_case_39_MODERATOR(app, request, user1, user2, freeuserrole, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with moderator role, user2 with FREEUSER role.
        ...
        # Log in with user1
        login_user(user1)
        # Test MODERATOR accessing demote_manager()
...
def test_case_40_MODERATOR(app, request, user1, user2, user3, freeuserrole, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with moderator role, user2, user3 with FREEUSER role.
...
def test_case_40_MODERATOR_1(app, request, user1, user2, user3, freeuserrole, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 MODERATOR role, user2, user3 with FREEUSER role.
        #           a public event owned by user3, managed by user2
...
def test_case_40_MODERATOR_2(app, request, user1, user2, user3, freeuserrole, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 MODERATOR role, user2, user3 with FREEUSER role.
        #           a public event owned by user3, managed by user2
...
def test_case_41_MODERATOR(app, request, user1, user2, freeuserrole, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with moderator role, user2 with FREEUSER role.
        ...
        # Log in with user1
        login_user(user1)
        # Test MODERATOR accessing accept_request()
...
def test_case_42_MODERATOR(app, request, user1, user2, user3, freeuserrole, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with moderator role, user2, user3 with FREEUSER role.
...
def test_case_43_MODERATOR(app, request, user1, user2, user3, freeuserrole, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with moderator role, user2, user3 with FREEUSER role.
        #           a public event owned by user3, 
        #           request to join by user2
...
def test_case_43_MODERATOR_1(app, request, user1, user2, user3, freeuserrole, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with moderator role, user2, user3 with FREEUSER role.
        #           a public event owned by user3, 
        #           request to join by user2
...
def test_case_44_MODERATOR(app, request, user1, user2, freeuserrole, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with moderator role, user2 with FREEUSER role.
        #           a public event owned by user2,
        #           attended by user1
...
def test_case_44_MODERATOR_1(app, request, user1, user2, moderatorrole, freeuserrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 MODERATOR role, user2 with FREEUSER role.
        #           a public event owned by user2,
        #           NOT attended by user1
...
def test_case_45_MODERATOR(app, request, user1, user2, freeuserrole, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with moderator role, user2 with FREEUSER role.
        #           a public event owned by user1, request to join by user2
        user1.role = moderatorrole
        ...
        # Log in with user1
        login_user(user1)
        # Test MODERATOR accessing reject_request()
        result = reject_request(user2.id, publicevent1.id)
        assert user2.id not in [m.id for m in publicevent1.attendants]
        assert user2.id not in [m.id for m in publicevent1.requesters]

#Test Case	46	Reject request to join manager
def test_case_46_MODERATOR(app, request, user1, user2, user3, freeuserrole, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with moderator role, user2, user3 with FREEUSER role.
        #           a public event owned by user3, 
        #           request to join by user2
...
#def test_case_47_MODERATOR(app, request, user1, user2, freeuserrole, moderatorrole, publicevent1):
#    with current_app.test_request_context():
#        # Scenario: person user1 with moderator role, user2 with FREEUSER role.
#        #           a public event owned by user2,
#        #           attended by user1
...
#        # Log in with user1
#        login_user(user1)
#        # Test MODERATOR accessing leave()
#        result = leave(publicevent1.id)
#        assert user1.id not in [m.id for m in publicevent1.attendants]

#def test_case_47_MODERATOR_1(app, request, user1, user2, freeuserrole, moderatorrole, publicevent1):
#    with current_app.test_request_context():
#        # Scenario: person user1 with moderator role, user2 with FREEUSER role.
#        #           a public event owned by user2,
#        #           attended by user1
...
#        # Log in with user1
#        login_user(user1)
#        # Test MODERATOR accessing remove_attendee()
#        result = remove_attendee(user1.id, publicevent1.id)
#        assert user1.id not in [m.id for m in publicevent1.attendants]

##Test Case	48	Remove others from attending an event, manager
def test_case_48_MODERATOR(app, request, user1, user2, user3, freeuserrole, 
                             moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with moderator role, user2, user3 with FREEUSER role.
        #           a public event owned by user2,
        #           managed by user1
...
def test_case_49_MODERATOR(app, request, user1, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with moderator role.
        #           a public event owned by user1
        user1.role = moderatorrole
        db.session.add(user1)
        publicevent1.owner = user1 
...
def test_case_50_MODERATOR(app, request, user1, user2, user3, freeuserrole, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with moderator role, user2, user3 with FREEUSER role.
        #           a public event owned by user2,
...
def test_case_51_MODERATOR(app, request, user1, user2, freeuserrole, 
                             moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with moderator role, user2 with FREEUSER role.
        #           a public event owned by user2
...
def test_case_52_MODERATOR(app, request, user1, user2, moderatorrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with moderator role, 
        #           user2 with PREMIUMUSER role.
        #           a private event owned by user2
        user1.role = moderatorrole
        db.session.add(user1)
        user2.role = premiumuserrole
        ...
        assert user1.id in [m.id for m in privateevent1.requesters]

#Test Case	53	Reject request to join, non manager, non owner, non requester
def test_case_53_MODERATOR(app, request, user1, user2, user3, freeuserrole, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with moderator role, user2, user3 with FREEUSER role.
        #           a public event owned by user3, 
        #           request to join by user2
...
#def test_case_54_MODERATOR(app, request, user1, user2, freeuserrole, moderatorrole, publicevent1):
#    with current_app.test_request_context():
#        # Scenario: person user1 with moderator role, user2 with FREEUSER role.
#        #           a public event owned by user2, 
...
# Test /manage_event
def test_case_56_MODERATOR(app, request, user1, user2, user3, freeuserrole, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
...
# Test \users
def test_case_56_MODERATOR_1(app, request, user1, user2, freeuserrole, moderatorrole):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role, user2 with FREEUSER role.
        user1.role = moderatorrole
        db.session.add(user1)
        ...
        # Log in with user1
        login_user(user1)
        # Test MODERATOR accessing users()
        result = users()
...
# Test \user
def test_case_56_MODERATOR_2(app, request, user1, user2, freeuserrole, moderatorrole):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role, user2 with FREEUSER role.
        user1.role = moderatorrole
        db.session.add(user1)
...
# Test edit_category
def test_case_56_MODERATOR_4(app, request, user1, moderatorrole,
                          category1, user2, user3):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role
        ...
        try:
            ...
            assert is_response(result) or \
                   (result.get("category").name == category1.name and \
                    ...
                    (len(result.get("candidates")) == 2 and \
                     (result.get("candidates")[0].fullname == user3.fullname and
                      result.get("candidates")[1].fullname == user1.fullname) or \
                     (result.get("candidates")[0].fullname == user1.fullname and
                      result.get("candidates")[1].fullname == user3.fullname)))
        except SecurityException:
            assert True

#Test Case	57	Read own core information
# Test /main
def test_case_57_MODERATOR(app, request, user1, moderatorrole):
    with current_app.test_request_context():
        # Scenario: a moderator user1
        user1.role = moderatorrole
        db.session.add(user1)
...
# Test /manage_event
def test_case_57_MODERATOR_1(app, request, user1, freeuserrole, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a public event owned by user1
        user1.role = moderatorrole
...
# Test \users
def test_case_57_MODERATOR_1(app, request, user1, moderatorrole):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        user1.role = moderatorrole
        db.session.add(user1)
...
# Test \user
def test_case_57_MODERATOR_2(app, request, user1, moderatorrole):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        user1.role = moderatorrole
        db.session.add(user1)
...
def test_case_58_MODERATOR(app, request, user1, moderatorrole):
    with current_app.test_request_context(
        data={"id": user1.id, 
              "name": "newname",
              "surname": "newsurname",
              "role": moderatorrole.name}):
        # Scenario: a user user1 with moderator role
        user1.role = moderatorrole
        db.session.add(user1)
        db.session.commit()
        # Log in with user1
...
def test_case_59_MODERATOR(app, request, user1, user2, freeuserrole, moderatorrole):
    with current_app.test_request_context(
        data={"id": user2.id, 
              "name": "newname",
              "surname": "newsurname",
...
# Test /profile
def test_case_61_MODERATOR(app, request, user1, freeuserrole, moderatorrole, premiumuserrole, 
                          user2, publicevent1, category1):
    with current_app.test_request_context():
        # Scenario: a user1 with moderator role, user2 with premiumrole
        #           a category category1
...
# Test /profile
def test_case_62_MODERATOR(app, request, user1, freeuserrole, moderatorrole,
                          user2, publicevent1, category1):
    with current_app.test_request_context():
        # Scenario: a user1 with moderator role, user2 with premiumrole
        #           a category category1
...
def test_case_63_MODERATOR(app, request, user1, moderatorrole, freeuserrole):
    with current_app.test_request_context(
        data={ "id": user1.id, 
              "name": user1.name,
              "surname": user1.surname,
        ...
        try:
            ...
            assert user1.role.name == moderatorrole.name

#Test Case	64	View requested access
def test_case_64_MODERATOR(app, request, user1, user2, moderatorrole, freeuserrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 moderator role, user2 with FREEUSER role.
        #           a public event owned by user1, request to join by user2
        user1.role = moderatorrole
...
def test_case_65_MODERATOR(app, request, user1, moderatorrole, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderatorrole role.
        #           a category1 subscribed by user1
        user1.role = moderatorrole
        db.session.add(user1)
        ...
        result = profile()
        subs_dto = result.get("subs")
        assert len(subs_dto) == 1
        assert subs_dto[0].name == category1.name

#Test Case	66	Unsubscribe from categories
def test_case_66_MODERATOR(app, request, user1, moderatorrole, category1):
    with current_app.test_request_context():
        # Scenario: a category category1
        #           a person user1 with moderator role.
        ...
        # Log in with user1
        login_user(user1)
        # Test MODERATOR accessing unsubscribe()
        result = unsubscribe(category1.id)
        assert user1.id not in [m.id for m in category1.subscribers]

#Test Case	67	Subscribe to categories
#Test Case	68	Add category to your subscriptions
def test_case_67_68_MODERATOR(app, request, user1, moderatorrole, category1):
    with current_app.test_request_context():
        # Scenario: a category category1
        #           a person user1 with moderator role.
        user1.role = moderatorrole
        ...
        # Log in with user1
        login_user(user1)
        # Test MODERATOR accessing subscribe()
        result = subscribe(category1.id)
        assert user1.id in [m.id for m in category1.subscribers]

#Test Case	69	Create a private event
def test_case_69_MODERATOR(app, request, user1, moderatorrole, privateevent1):
    with current_app.test_request_context(
        ...
        events = Event.query.all()
        assert len(events) == 1
        assert events[0].title == privateevent1.title
        assert events[0].description == privateevent1.description
        assert events[0].private
        assert events[0].categories == privateevent1.categories

#Test Case	70	Remove category they do not moderate from event
def test_case_70_MODERATOR(app, request, user1, user2, moderatorrole, publicevent1, category1):
    with current_app.test_request_context():
        # Scenario: person user1, user2 with moderator role.
        #           a category category1
        #           a public event owned by user2, of category1
        user1.role = moderatorrole
        db.session.add(user1)
        ...
        # Log in with user1
        login_user(user1)
        # Test MODERATOR accessing remove_category()
        try:
...
def test_case_70_MODERATOR_1(app, request, user1, user2, moderatorrole, publicevent1, category1):
    with current_app.test_request_context(
        data={ "id": publicevent1.id, 
...
def test_case_72_MODERATOR(app, request, user1, moderatorrole, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a category category1, moderated by user1
        user1.role = moderatorrole
        db.session.add(user1)
        ...
        # Log in with user1
        login_user(user1)
        # Test MODERATOR accessing remove_moderator()
...
def test_case_70_MODERATOR(app, request, user1, moderatorrole, publicevent1, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a category category1, moderated by user1
        #           a public event owned by user1, of category1
...
def test_case_74_MODERATOR(app, request, user1, premiumuserrole, 
                               moderatorrole, category1, user2):
    with current_app.test_request_context():
        # Scenario: a person user1 with moderator role.
        #           a person user2 with PREMIUMUSER role.
        #           a category category1
        ...
        assert len(category_dto.subscribers) == 1 and \
               category_dto.subscribers[0].fullname == user2.fullname

def test_case_74_MODERATOR_1(app, request, user1, 
                               moderatorrole, category1, 
                               user2, premiumuserrole):
    with current_app.test_request_context():
        # Scenario: a person user1 with MODERATOR role.
        #           a person user2 with PREMIUMUSER role.
        #           a category category1
        #           both users subscribe to category1
        user1.role = moderatorrole
        db.session.add(user1)
        user2.role = premiumuserrole
        db.session.add(user2)
        ...
        # Log in with user1
        login_user(user1)
        # Test MODERATOR access to profile()
        result = profile()
        subs_dto = result.get("subs")
        assert len(subs_dto) == 1
        assert len(subs_dto[0].subscribers) != 2

def test_case_74_MODERATOR_2(app, request, user1, 
                               moderatorrole, category1, 
                               user2, premiumuserrole):
    with current_app.test_request_context():
        # Scenario: a person user1 with MODERATOR role.
...
#              "role": premiumuserrole.name}):
#        # Scenario: a user user1 with moderator role, user2 with FREEUSER role
#        user1.role = moderatorrole
#        db.session.add(user1)
#        user2.role = freeuserrole
#        db.session.add(premiumuserrole)
...
def test_case_78_MODERATOR(app, request, user1, user2, moderatorrole, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with MODERATOR role.
        #           a person user2 with MODERATOR role.
        #           a category category1
        ...
        # Log in with user1
        login_user(user1)
        # Test MODERATOR accessing add_moderator()
        try:
...
def test_case_79_MODERATOR(app, request, user1, user2, moderatorrole, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with MODERATOR role.
        #           a person user2 with MODERATOR role.
...
def test_case_79_MODERATOR_1(app, request, user1, user2, moderatorrole, category1):
    with current_app.test_request_context():
        # Scenario: a person user1 with MODERATOR role.
        #           a person user2 with MODERATOR role.
        #           a category category1, moderated by user1 and user2
        user1.role = moderatorrole
        db.session.add(user1)
        user2.role = moderatorrole
        db.session.add(user2)
...
def test_case_80_MODERATOR(app, request, user1, moderatorrole, category1):
    with current_app.test_request_context(
        ...
        # Log in with user1
        login_user(user1)
        # Test MODERATOR accessing create_category()
        try:
...
def test_case_82_MODERATOR(app, request, user1, moderatorrole, category1):
    with current_app.test_request_context(
        ...
        # Log in with user1
        login_user(user1)
        # Test MODERATOR accessing update_category()
        try:
...
def test_case_83_MODERATOR(app, request, user1, user2, user3, freeuserrole, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with MODERATOR role, user2, user3 with FREEUSER role.
        #           a public event owned by user3
        #           managed by user1
...
def test_case_84_MODERATOR(app, request, user1, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with moderator role.
        #           a public event owned by user1
        user1.role = moderatorrole
...
# leave functionality is incorrectly implemented, waive this test
#def test_case_86_MODERATOR(app, request, user1, user2, freeuserrole, moderatorrole, publicevent1):
#    with current_app.test_request_context():
#        # Scenario: person user1 with MODERATOR role, user2 with FREEUSER role.
#        #           a public event owned by user2, managed by user1
#        user1.role = moderatorrole
...
def test_case_86_MODERATOR_1(app, request, user1, user2, freeuserrole, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with MODERATOR role, user2 with FREEUSER role.
        #           a public event owned by user2, managed by user1
        user1.role = moderatorrole
...
def test_case_87_MODERATOR(app, request, user1, user2, freeuserrole, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 with MODERATOR role, user2 with FREEUSER role.
        #           a public event owned by user2, managed by user1
        user1.role = moderatorrole
...
def test_case_90_MODERATOR(app, request, user1, user2, user3, freeuserrole, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: person user1 moderator role, user2, user3 with FREEUSER role.
        ...
        # Log in with user1
        login_user(user1)
        # Test moderatorrole accessing accept_request()
        try:
            result = accept_request(user2.id, publicevent1.id)
        except SecurityException:
            assert True
...
def test_case_91_MODERATOR(app, request, user1, moderatorrole, user2, freeuserrole, publicevent1, category1):
    with current_app.test_request_context(
        data={ "id": publicevent1.id, 
              "title": publicevent1.title, 
              "description": publicevent1.description, 
              "categories": []}):
        # Scenario: a person user1 moderator role, user2 with FREEUSER role.
        #           a public event owned by user2, managed by user1
        user1.role = moderatorrole
        db.session.add(user1)
        user2.role = freeuserrole
        ...
        # Log in with user1
        login_user(user1)
        # Test moderatorr accessing update_event()
        result = update_event()
        assert len(publicevent1.categories) == 0

#Test Case	92	Edit event's categories, not manager
def test_case_92_MODERATOR(app, request, user1, moderatorrole, user2, freeuserrole, publicevent1, category1):
    with current_app.test_request_context(
        data={ "id": publicevent1.id, 
              "title": publicevent1.title, 
              "description": publicevent1.description, 
...
def test_case_93_MODERATOR(app, request, user1, user2, moderatorrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with MODERATOR role.
        #           a person user2 with PREMIUMUSER role.
        #           a private event owned by user2
...
def test_case_93_MODERATOR_1(app, request, user1, user2, moderatorrole, premiumuserrole, privateevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with MODERATOR role.
        #           a person user2 with PREMIUMUSER role.
        #           a private event owned by user2
...
def test_case_94_MODERATOR(app, request, user1, moderatorrole, publicevent1):
    with current_app.test_request_context():
        # Scenario: a person user1 with MODERATOR role.
        #           a public event owned by user1
        user1.role = moderatorrole
